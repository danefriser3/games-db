...
const TopPopular = ({ access_token }: { access_token: string }) => {
  const [platform, setPlatform] = useState(6);
  const { data, isLoading } = useSWR(
    [access_token, platform, 2],
    ([access_token, platform]) =>
      GamesController.getTop10Popular(access_token, platform)
  );
  return (
    <div className="flex flex-col gap-4 w-full lg:w-4/5 rounded-lg p-5 bg-slate-400">
      <div className="flex flex-row gap-3 items-center">
        <div className="text-start text-xl font-bold">Top 100 popolari</div>
